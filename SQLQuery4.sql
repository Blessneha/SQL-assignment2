...
create table Department(Dept_no int not null primary key,Deptname varchar(20),location varchar(20));
drop table Department
create table Department(Dept_no varchar(8) not null primary key,Deptname varchar(20),location varchar(20));
insert into Department values('d1','Research','Dallas')
insert into Department values('d2','Accounting','Seattle')
insert into Department values('d3','Marketing','Dallas')

create table Employee(emp int not null,emp_fna varchar(20),emp_lna varchar(20),Dept_no varchar(8),primary key(emp),foreign key(Dept_no) references Department(Dept_no))
...
create table Works_on(emp_no int,project_no varchar(8),Job varchar(20),
enter_date date,foreign key(emp_no) references Employee(emp),foreign key(project_no) references Project(project_no))
...
insert into Works_on values(25348,'p3','Analyst','1998.10.15')
insert into Works_on values(18316,'p1','Manager','1998.4.15')
insert into Works_on values(29346,'p1',NULL,'1998.8.1')
insert into Works_on values(29346,'p2','Clerk','1992.2.1')
insert into Works_on values(18316,'p3','Clerk','1997.11.15')
...
select distinct e.emp_fna,e.emp_lna from Employee e right join Works_on w1 on e.emp=w1.emp_no inner join Works_on w2 on w1.enter_date=w2.enter_date

select distinct e.emp from Employee e inner join Department d1 on e.Dept_no=d1.Dept_no inner join Department d2 on d1.location=d2.location
...
create table emp_d1_d2(emp int not null,emp_fna varchar(20),emp_lna varchar(20),Dept_no varchar(8),primary key(emp),foreign key(Dept_no) references Department(Dept_no))
